...
def handle_checkup(whatsapp_number, user_id, supabase, user_data, message):
    state = user_data.get(whatsapp_number, {}).get("state", "IDLE")
    logger.info(f"Handling checkup for {whatsapp_number}, state: {state}")
    ...
    if state == "IDLE":
        ...
        service_name = user_data[whatsapp_number].get("service_name", translate_template(whatsapp_number, "Checkup", supabase))
        ...
        # Get category image for Checkup & Test
        category_image_url = None
        try:
            response = supabase.table("c_a_clinic_cat").select("image_url") \
                .eq("clinic_id", clinic_id).eq("category", "Checkup & Test").limit(1).execute()
        ...
        try:
            ...
            logger.info(f"Checkup service details response for {service_id}: {resp.data}")
            ...
            logger.error(f"Error fetching checkup service details for {service_id}: {e}", exc_info=True)
            duration = 30
            reminder_duration = None
            reminder_remark = None
            doctor_id = None
        ...
        user_data[whatsapp_number] = {
            "state": "CHECKUP_REMARK_YES_NO",
            "module": "checkup_booking",
            "language": language,
            "clinic_id": clinic_id,
            "service_id": service_id,
            "checkup_type": service_name,  # Original service name
            "display_checkup_type": service_name,  # Display name
        ...
        if description.strip():
            # Use gt_tt for dynamic description text
        ...
        prompt_text = translate_template(
            whatsapp_number,
            "Do you have any remarks for {} ({} min){}?", 
            supabase
        ).format(
            service_name,  # Keep original service name
            duration,
    ...
    # -------------------------------------------------------------------------
    # CHECKUP_REMARK_YES_NO
    # -------------------------------------------------------------------------
    elif state == "CHECKUP_REMARK_YES_NO" and message.get("type") == "interactive":
        if message["interactive"]["button_reply"]["id"] == "remark_yes":
            user_data[whatsapp_number]["state"] = "CHECKUP_REMARK_INPUT"
            send_whatsapp_message(
                whatsapp_number,
                "text",
            ...
            user_data[whatsapp_number]["details"] = user_data[whatsapp_number]["checkup_type"]
            user_data[whatsapp_number]["state"] = "SELECT_DOCTOR"
            get_doctors(whatsapp_number, user_id, supabase, user_data, "checkup_booking")
        return False

    # -------------------------------------------------------------------------
    # CHECKUP_REMARK_INPUT
    # -------------------------------------------------------------------------
    elif state == "CHECKUP_REMARK_INPUT" and message.get("type") == "text":
        remark = message["text"]["body"].strip()
        user_data[whatsapp_number]["details"] = f"{user_data[whatsapp_number]['checkup_type']}: {remark}"
        user_data[whatsapp_number]["state"] = "SELECT_DOCTOR"
        get_doctors(whatsapp_number, user_id, supabase, user_data, "checkup_booking")
        return False
    ...
    # -------------------------------------------------------------------------
        ...
        if selected != "any_doctor":
            try:
                ...
                if doctor_response.data:
                    user_data[whatsapp_number]["selected_doctor_name"] = doctor_response.data[0]["name"]  # Keep original name
        ...
        get_calendar(whatsapp_number, user_id, supabase, user_data, "checkup_booking")
        return False
    ...
    # -------------------------------------------------------------------------
        ...
        if selected_date == "future_date":
            ...
            # NEW: Instead of going directly to SELECT_PERIOD, ask for time input
    ...
    # -------------------------------------------------------------------------
    # AWAITING_TIME_INPUT (NEW STATE)
    # -------------------------------------------------------------------------
    elif state == "AWAITING_TIME_INPUT" and message.get("type") == "text":
        time_input = message["text"]["body"].strip()
        handle_time_input(whatsapp_number, user_id, supabase, user_data, "checkup_booking", time_input)
        return False

    # -------------------------------------------------------------------------
    # CONFIRM_TIME (NEW STATE)
    # -------------------------------------------------------------------------
    elif state == "CONFIRM_TIME" and message.get("type") == "interactive":
        button_id = message["interactive"]["button_reply"]["id"]
        if button_id == "confirm_time":
            handle_time_confirmation(whatsapp_number, user_id, supabase, user_data, "checkup_booking", confirmed=True, use_closest=False)
            ...
            select_period(whatsapp_number, user_id, supabase, user_data, "checkup_booking")
        return False

    # -------------------------------------------------------------------------
    # CONFIRM_CLOSEST_TIME (NEW STATE)
    # -------------------------------------------------------------------------
    elif state == "CONFIRM_CLOSEST_TIME" and message.get("type") == "interactive":
        button_id = message["interactive"]["button_reply"]["id"]
        if button_id == "accept_closest_time":
            handle_time_confirmation(whatsapp_number, user_id, supabase, user_data, "checkup_booking", confirmed=True, use_closest=True)
        elif button_id == "find_another_time":
            # User wants to find another time - go to AM/PM selection
            user_data[whatsapp_number].pop("parsed_time_input", None)
            user_data[whatsapp_number].pop("closest_available_time", None)
    ...
    # -------------------------------------------------------------------------
        ...
        handle_future_date_input(whatsapp_number, user_id, supabase, user_data, "checkup_booking", date_input)
        return False
    ...
    # -------------------------------------------------------------------------
        ...
        if button_id == "confirm_future_date":
            # Modified to go to time input instead of period selection
            ...
            if date_obj:
                ...
                # Go to time input instead of period selection
                user_data[whatsapp_number]["state"] = "AWAITING_TIME_INPUT"
                send_whatsapp_message(
                    whatsapp_number,
                    "text",
            ...
            handle_future_date_confirmation(whatsapp_number, user_id, supabase, user_data, "checkup_booking", confirmed=False)
        return False

    # -------------------------------------------------------------------------
    # SELECT_PERIOD (FALLBACK - when user chooses "Help Me Choose" or "Find Another")
    # -------------------------------------------------------------------------
        ...
        get_available_hours(whatsapp_number, user_id, supabase, user_data, "checkup_booking")
        return False
        ...
        get_time_slots(whatsapp_number, user_id, supabase, user_data, "checkup_booking")
        return False
        ...
        get_available_doctors(whatsapp_number, user_id, supabase, user_data, "checkup_booking")
        return False

    # -------------------------------------------------------------------------
    # CONFIRM_BOOKING - Save to pending_bookings (same as healthsp.py)
    # -------------------------------------------------------------------------
        ...
        if button_id == "confirm_booking":
            # Get data
            checkup_type = user_data[whatsapp_number]["checkup_type"]
            ...
            details = user_data[whatsapp_number].get("details", checkup_type)
            
            # Create booking in pending_bookings (same structure as healthsp.py)
            try:
                ...
                logger.info(f"Checkup booking saved to pending: {pending_id} for {whatsapp_number}")
                ...
                confirmation_message = translate_template(
                    ...
                    checkup_type,  # Keep original service name
                    date,
                    time_slot,
                    duration_minutes,
                    pending_id[:8]
                ...
                logger.error(f"Error saving checkup booking for {whatsapp_number}: {e}", exc_info=True)
                send_whatsapp_message(
                    whatsapp_number,
                    "text",
                    {"text": {"body": translate_template(
                        whatsapp_number,
                        "Error saving checkup booking. Please try again.", 
                        supabase
                    )}},
                    supabase
                )
            ...
            # Handle edit booking
            show_edit_options(whatsapp_number, user_id, supabase, user_data, "checkup_booking")
            return False
    ...
    # -------------------------------------------------------------------------
        ...
        if "list_reply" in message["interactive"]:
            ...
            handle_edit_choice(whatsapp_number, user_id, supabase, user_data, "checkup_booking", edit_choice)
        return False
    ...
    # -------------------------------------------------------------------------
        ...
        if current_state in ["AWAITING_FUTURE_DATE", "CHECKUP_REMARK_INPUT", "AWAITING_TIME_INPUT"]:
            ...
            elif current_state == "CHECKUP_REMARK_INPUT":
                send_whatsapp_message(
                    whatsapp_number,
                    "text",
                    {"text": {"body": translate_template(whatsapp_number, "Please enter your remarks:", supabase)}},
