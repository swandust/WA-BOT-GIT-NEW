# main.py - UPDATED WITH FIX FOR VIEW_BOOKING FUTURE DATE HANDLING
import uuid
import threading
import time
import logging
                ...
                "sections": [
                    {
                        ...
                        "rows": [
                            ...
                            {"id": "back_button", "title": translate_template(whatsapp_number, "🔙 Back to Main Menu", supabase)}  # CHANGED from gt_tt
                        ]
                    }
                ]
            }
...
def handle_message(value):
    ...
    try:
        ...
        if message["type"] == "location":
            ...
            if module in ["ambulance_booking", "ambulance_homehome", "tcm_service"]:
            ...
            if button_id.startswith("book_at_"):
                ...
                try:
                    ...
                    send_whatsapp_message(
                        whatsapp_number,
                        "text",
                        {"text": {"body": "An error occurred while setting up your booking. Please try again."}}
                    )
                    user_data[whatsapp_number] = {"state": "IDLE", "processing": False, "module": None}
                    send_interactive_menu(whatsapp_number, supabase)
                return
        ...
        if state == "SELECT_LANGUAGE" and message["type"] == "interactive" and message["interactive"].get("type") == "list_reply":
            ...
            if lang_id == "back_button":
                ...
                send_whatsapp_message(
                    whatsapp_number,
                    "text",
                    {"text": {"body": translate_template(whatsapp_number, "Returning to main menu.", supabase)}}  # CHANGED from gt_tt
                )
                send_interactive_menu(whatsapp_number, supabase)
                return
            ...
            try:
                ...
                send_whatsapp_message(
                    whatsapp_number,
                    "text",
                    {"text": {"body": translate_template(whatsapp_number, "Language set to {}.", supabase).format(selected_language)}}
                )
        ...
        if message["type"] == "text":
            ...
            📞 *Phone:* {clinic_data.get('phone_number', 'Not available')}
                            ...
                            send_whatsapp_message(
                                whatsapp_number,
                                "text",
                                {"text": {"body": "Clinic information not found. Please try again."}}
                            )
                        ...
                        send_whatsapp_message(
                            whatsapp_number,
                            "text",
                            {"text": {"body": "Error retrieving clinic information. Please try again."}}
                        )
                else:
                    ...
                    send_whatsapp_message(
                        whatsapp_number,
                        "text",
                        {"text": {"body": "No clinic found with that keyword. Please try a different search."}}
                    )
