# main.py - UPDATED WITH CORRECT TRANSLATION FUNCTIONS
import uuid
import threading
import time
import logging
...
def handle_message(value):
    ...
    try:
        ...
        # Note: This is handled in webhooksplit.py directly, but kept here as fallback
        if message["type"] == "interactive" and message["interactive"].get("type") == "button_reply":
            button_id = message["interactive"]["button_reply"]["id"]
            if button_id == "notification_noted":
                # Handle notification noted (this should normally be handled in webhook)
        ...
        # In your main.py, in the location handling section:
        if message["type"] == "location":
            logger.info(f"Received location message from {whatsapp_number}")
       
            # Check if we're in ambulance booking state that expects location
            if module == "ambulance_booking" and state == "BOOKING_PICKUP_ADDRESS_LOCATION":
            ...
            # For other modules that might expect location
            if module in ["ambulance_booking", "ambulance_homehome", "tcm_service", "ambulance_discharge", "ambulance_hosphosp"]:
        ...
        if state == "SELECT_LANGUAGE" and message["type"] == "interactive" and message["interactive"].get("type") == "list_reply":
            ...
            try:
                ...
                send_whatsapp_message(
                    whatsapp_number,
                    "text",
                    {"text": {"body": gt_tt(whatsapp_number, f"Language set to {selected_language}.", supabase)}}
                )
        ...
        if message["type"] == "text":
            ...
            if user_text_lower.startswith("find_"):
                from utils import lookup_clinic_by_keyword, get_user_language
                ...
                if clinic_info:
                    ...
                    try:
                        ...
                        if clinic_details.data and len(clinic_details.data) > 0:
                            ...
                            # Send clinic details message (clinic name and address remain in English)
                            clinic_message = f"""
...
📞 *Phone:* {clinic_data.get('phone_number', 'Not available')}
                            """.strip()
                            ...
                            text_payload = {
                                ...
                                "type": "text",
                                "text": {
                                    "body": gt_tt(whatsapp_number, clinic_message, supabase)
                                }
                            }
                            ...
                            booking_prompt = "Would you like to book an appointment at this clinic?"
                            booking_payload = {
                                "messaging_product": "whatsapp",
                                "to": whatsapp_number,
                                "type": "interactive",
                                "interactive": {
                                    "type": "button",
                                    "body": {
                                        "text": translate_template(whatsapp_number, booking_prompt, supabase)
                                    },
                                    "action": {
                                        "buttons": [
                                            {
                            ...
                            send_whatsapp_message(
                                whatsapp_number,
                                "text",
                                {"text": {"body": translate_template(whatsapp_number, "Clinic information not found. Please try again.", supabase)}}
                            )
                        ...
                        send_whatsapp_message(
                            whatsapp_number,
                            "text",
                            {"text": {"body": translate_template(whatsapp_number, "Error retrieving clinic information. Please try again.", supabase)}}
                        )
                else:
                    ...
                    send_whatsapp_message(
                        whatsapp_number,
                        "text",
                        {"text": {"body": translate_template(whatsapp_number, "No clinic found with that keyword. Please try a different search.", supabase)}}
                    )
